...
void RealTimeParser::processQueue()
{
    ...
    while (true)
    {
        ...
        // qDebug() << "  pt.depth:" << pt.depth << "   pt.speed:" << pt.speed << "  pt.time:" << pt.time << "  pt.date:" << pt.date;
        emit parsedPoint(pt);
    }
...
BLEManager::BLEManager(QObject *parent) : QObject(parent)
{
    ...
    loadingQuickView_->setSource(QUrl("qrc:/QML/App/loading.qml"));
...
void BLEManager::BleServiceCharacteristicChanged(const QLowEnergyCharacteristic &c,const QByteArray &value)
{
    ...
    // lastElapsed_ = now;
    // qDebug() << "view BLEManager ms: " << interval << "  count: " << bleCount_;
    Q_UNUSED(c)
